...
public class EarthquakeCityMap extends PApplet {	
	...
	*/
	...
	public void mouseMoved(){		
		if (lastSelected != null) {   // clear the last selection
	...
	private void selectMarkerIfHover(List<Marker> markers){
		...
		for (Marker m: markers){
			if(m.isInside(map,x,y) && lastSelected==null){
	 ...
	 *  Or if a city is clicked, it will display all the earthquakes where the city is in the threat circle
	 */
	@Override
	...
	private void mouseClickHelper(Marker select){
		Location loc = select.getLocation();
		
		if(select instanceof EarthquakeMarker){
			for(Marker cm: cityMarkers){
				double dist = ((EarthquakeMarker)select).threatCircle();
				if(cm.getDistanceTo(loc)> dist){
					cm.setHidden(true);
				}				
			}
		...
		if(select instanceof CityMarker){
			for(Marker qm: quakeMarkers){
				if(qm.getDistanceTo(loc)>((EarthquakeMarker)qm).threatCircle()) {
					qm.setHidden(true);
				}
			}
			for(Marker cm: cityMarkers){
